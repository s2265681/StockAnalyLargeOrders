...
REGROUP_PROMPT = """你是A股超短题材复盘专家。下面这些涨停股被初步归为「其他概念」，说明它们没有被归入主流大主线。

请你重新审视这批股票，尝试从中归纳出 1-4 个有意义的细分标签（2-8字，市场复盘常用语言）。

要求：
1. 每个新标签必须至少包含 2 只股票，不满 2 只的继续放「其他概念」。
2. 标签名必须是市场认可的方向，例如「低空经济」「医疗器械」「军工」「船舶」「储能」等，不允许过细如「某某公司概念」。
3. 尽量把所有股票归类，减少「其他概念」数量，但不要强行合并不相关的票。
4. 每个新标签写一句涨停原因，选出 1 个龙头。
5. 已有的主流大标签列表如下，不要和它们重名或创造语义相近的新标签：
{existing_labels}
...
def _smart_group_stocks(stocks: list, dt: str) -> dict:
    ...
    # 5. 「其他概念」超过 5 只时，二次调用 Claude 归纳新标签
    OTHER_LABELS = {"其他概念", "其他", "其他行业"}
    other_codes = [code for code, lbl in labels.items() if lbl in OTHER_LABELS]
    if len(other_codes) > 5:
        logger.info(f"「其他概念」共 {len(other_codes)} 只，触发二次归纳...")
        stock_map = {s["code"]: s for s in stocks}
        other_stocks = [stock_map[c] for c in other_codes if c in stock_map]
        existing_labels_text = "、".join(lbl for lbl in set(labels.values()) if lbl not in OTHER_LABELS)
            ...
            if regroup.get("labels"):
                ...
                if regroup.get("leaders"):
                    for tag, ldr in regroup["leaders"].items():
                        if tag not in leaders:
                            leaders[tag] = ldr
                remaining = sum(1 for lbl in labels.values() if lbl in OTHER_LABELS)
...
def get_limit_up_echelon():
    ...
    try:
        ...
        # 3. AI 分组：始终尝试读取 DB/内存缓存；只有 force=1 才触发新 AI 任务
        theme_ranking = []
            ...
            if group_result.get("labels"):
                ...
                with _ai_task_lock:
                    ...
                    if task and task["status"] == "done" and task["result"].get("labels"):
                        ...
                        t.start()
                        ai_meta["status"] = "pending"
            # 否则：无 DB 数据 + 无 force → status='none'，不触发 AI
