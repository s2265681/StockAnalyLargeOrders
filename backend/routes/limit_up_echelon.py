...
3. 每只股票必须只属于一个大标签。归类时重点参考同花顺给出的概念标签和涨停标题，找到该票与哪个大标签关联最紧密。
4. **「其他概念」必须控制在 3 只以内**。只有完全无法与任何大标签关联的个股才放「其他概念」。如果某只票的同花顺概念、行业、涨停标题中能找到与任一大标签的关联（哪怕是次要关联），就必须归入那个大标签，而非「其他概念」。宁可放进关联性较弱的大标签，也不要轻易扔进「其他概念」。
5. 每个大标签必须给一句「涨停原因」，像短线复盘语言，概括该主题当天爆发原因，例如"特斯拉Optimus Gen-3启动量产，Figure机器人连续工作30小时"。
6. 每个大标签选出 1-2 个「板块龙头」，优先只选 1 个。按顺序输出：最正宗、最有带动性的先锋龙放第一；如需要，再给一个容量/辨识度最高的中军放第二。优先考虑连板高度、封板强度、同花顺热度、市场辨识度、题材正宗性和带动性。
7. 按成分股数量从高到低输出 groups，「其他概念」放最后。
...
def _bg_ai_grouping(stocks: list, cache_key: tuple):
    ...
    try:
        ...
        logger.error(f"后台AI分组失败: {e}", exc_info=True)
        with _ai_task_lock:
            _ai_task_status[cache_key] = {"status": "error", "result": {}}
