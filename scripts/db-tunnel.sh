...
_ssh_args=(-p "$SERVER_PORT" -o ServerAliveInterval=30 -o ServerAliveCountMax=3 -o ExitOnForwardFailure=yes)
if [ -n "$SSH_KEY" ]; then
  _ssh_args+=(-i "$SSH_KEY")
fi
...
_start() {
  ...
  if _tunnel_running; then
    ...
    echo "❌ 隧道启动失败，请检查 SSH 密钥能否登录 ${SERVER_USER}@${SERVER_HOST}" >&2
    echo "   可在 scripts/local.deploy.env 中设置 SERVER_USER / SSH_KEY" >&2
    exit 1
  fi
}
